    ...
    if (archiveEntry && Array.isArray(data.archives)) {
      ...
      if (!item) {
        ...
        archiveEntry.innerHTML = `
          ...
          ${item.manuscript_image ? `<figure class="archive-scroll-view"><a href="${esc(item.manuscript_image)}" target="_blank" rel="noopener" aria-label="Open ${esc(item.title)} at full size"><img src="${esc(item.manuscript_image)}" alt="${esc(item.hero_alt || '')}"></a><figcaption>Select the manuscript to open the full-size record.</figcaption></figure>` : (item.hero_image ? `<figure class="archive-hero"><img src="${esc(item.hero_image)}" alt="${esc(item.hero_alt || '')}"></figure>` : '')}

/**
 * Dark Lair Trilogy sound engine, version 3.4.
...
(function () {
  ...
  const OPENING_NAVIGATION_DELAY = 700;
  const OPENING_HANDOFF_OVERLAP = 0.48;
  const OPENING_VOLUME_MULTIPLIER = 2.0;
  const defaults = {
    enabled: true,
    master: 0.45,
    ambience: 0.55,
  ...
  class SoundEngine {
    ...
    play(src) {
      ...
      const audio = this.retainEffect(this.makeAudio(src, false, 'effects'));
      audio.play().catch(() => {});
      return audio;
    }

    setOpeningVolume(audio) {
      if (!audio) return audio;
      // Boost only the manuscript-opening effect at playback. The original WAV
      // remains completely untouched, preserving its natural character.
      audio.volume = Math.min(1, Math.max(0, this.volume('effects') * OPENING_VOLUME_MULTIPLIER));
      return audio;
    }

    playParchment() {
      const audio = this.retainEffect(this.makeAudio(OPENING_SOUND, false, 'effects'));
      this.setOpeningVolume(audio);
      audio.play().catch(() => {});
      return audio;
    }

    resumeParchmentAfterNavigation() {
      if (!this.settings.enabled) {
        sessionStorage.removeItem(OPENING_HANDOFF_KEY);
        return;
      }

      let handoff;
      try {
        handoff = JSON.parse(sessionStorage.getItem(OPENING_HANDOFF_KEY) || 'null');
      } catch {
        handoff = null;
      }
      sessionStorage.removeItem(OPENING_HANDOFF_KEY);

      if (!handoff || typeof handoff.startedAt !== 'number') return;
      const elapsed = (Date.now() - handoff.startedAt) / 1000;
      if (elapsed < 0 || elapsed > 5) return;

      const audio = this.retainEffect(this.makeAudio(OPENING_SOUND, false, 'effects'));
      this.setOpeningVolume(audio);
      const resume = () => {
        // Resume slightly before the calculated hand-off point. The intentional
    ...
    initialiseTriggers() {
      ...
      if (page === 'archives') {
        document.addEventListener('click', (event) => {
          ...
          sessionStorage.setItem(OPENING_HANDOFF_KEY, JSON.stringify({ startedAt: Date.now() }));
          this.playParchment();

          window.setTimeout(() => {
            window.location.href = destination;
          }, OPENING_NAVIGATION_DELAY);
        });
      }

      if (page === 'archive') {
        this.resumeParchmentAfterNavigation();
      }
    }
  }
